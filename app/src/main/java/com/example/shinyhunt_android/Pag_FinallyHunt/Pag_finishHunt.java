...
public class Pag_finishHunt extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pagfinishhunt);

        listView = findViewById(R.id.lista_Stop_Hunt);
            ...
            pokemonsRef.get().addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    ...
                    for (QueryDocumentSnapshot pokemonDocument : task.getResult()) {
                        ...
                        huntsRef.whereEqualTo("StatusHunt", "finalizada")
                                .get()
                                .addOnCompleteListener(huntTask -> {
                                    if (huntTask.isSuccessful()) {
                                        ...
                                        Toast.makeText(Pag_finishHunt.this, "Error getting hunts", Toast.LENGTH_SHORT).show();
                                    }
                                });
                    }
                } else {
                    Log.e(TAG, "Error getting pokemons: ", task.getException());
                    Toast.makeText(Pag_finishHunt.this, "Error getting pokemons", Toast.LENGTH_SHORT).show();
                }
            });
        } else {
            Log.e(TAG, "User not logged in");
            Toast.makeText(Pag_finishHunt.this, "User not logged in", Toast.LENGTH_SHORT).show();
        }
    }
}
